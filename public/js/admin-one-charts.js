 ...
 */
class AdminOneChartManager {    constructor() {
     ...
     * Create performance chart with enhanced sizing
     */
    createPerformanceChart(canvasId, data, options = {}) {
        const ctx = document.getElementById(canvasId);
        if (!ctx) return null;

        // Set explicit canvas dimensions for consistency
        const canvas = ctx;
        const isMainChart = canvasId.includes('mainChart');
        ...
        const chartConfig = {
            ...
            options: {
                ...
                onResize: (chart, size) => {
                    // Maintain fixed height on resize
                    if (isMainChart) {
    ...
    /**
     * Handle window resize to maintain chart dimensions
     */
    handleResize() {
        this.chartInstances.forEach((chart, canvasId) => {
            ...
            // Force chart resize with correct dimensions
            chart.resize();
        });
    }
     ...
     */
    refreshAllCharts(signalId, newData) {
        console.log('Refreshing all charts for signal:', signalId);
        
        // Destroy existing charts
        this.destroyChartsForSignal(signalId);
    ...
    /**
     * Destroy charts for a specific signal
     */
    destroyChartsForSignal(signalId) {
        const chartIds = [
            `mainChart_${signalId}`,
            `rsiChart_${signalId}`,
            `macdChart_${signalId}`,
            `volumeChart_${signalId}`
        ];
