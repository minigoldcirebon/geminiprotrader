...
// Test route with simulated auth (temporary - remove after testing)
...
// Direct test route for expert signals (bypasses auth middleware temporarily)
Route::get('/public-expert-signals', [ExpertSignalController::class, 'index'])->name('public-expert-signals');

// Public Expert Signals Route (no authentication required for public viewing)
Route::get('/expert-signals', [ExpertSignalController::class, 'index'])->name('expert-signals.public');

// Debug route to see controller data
Route::get('/debug-expert-data', function () {
    ...
    try {
        ...
        $debug = [
            'latestSignal' => $data['latestSignal'] ? [
                'id' => $data['latestSignal']->id,
                'pair' => $data['latestSignal']->pair,
                'signal_type' => $data['latestSignal']->signal_type,
                'status' => $data['latestSignal']->status,
                'chart_data_count' => $data['latestSignal']->chartDataPoints ? $data['latestSignal']->chartDataPoints->count() : 0
            ] : null,
            'otherSignals_count' => $data['otherSignals']->count(),
...
Route::get('/basic-expert-signals', function () {
    $controller = new \App\Http\Controllers\ExpertSignalController();
    $request = new \Illuminate\Http\Request();
    ...
    try {
        ...
        return view('expert-signals.index-basic', $data);
    } catch (Exception $e) {
        return "Error: " . $e->getMessage();
    }
});

// Debug route for expert signals
Route::get('/expert-signals-debug', function () {
    $controller = new ExpertSignalController();
    $request = new \Illuminate\Http\Request();
    $response = $controller->index($request);
    $data = $response->getData();
    
    return view('expert-signals.debug', $data);
});

// Debug route for expert signals without auth
Route::get('/expert-signals-no-auth', [ExpertSignalController::class, 'index']);

// Full debug route for expert signals
Route::get('/expert-signals-debug-full', function () {
    $controller = new ExpertSignalController();
    $request = new \Illuminate\Http\Request();
    $response = $controller->index($request);
    $data = $response->getData();
    
    return view('expert-signals.debug-full', $data);
});
...
// Layout test with expert data
Route::get('/layout-test-expert', function () {
...
// Expert Signal Routes (authenticated operations only)
...
Route::middleware(['auth', 'role:admin'])->prefix('admin')->name('admin.')->group(function () {
    ...
    Route::prefix('settings')->name('settings.')->group(function () {
        ...
        Route::put('/payment', [SystemSettingsController::class, 'updatePayment'])->name('payment.update');        Route::get('/email', [SystemSettingsController::class, 'email'])->name('email');
        Route::put('/email', [SystemSettingsController::class, 'updateEmail'])->name('email.update');
        Route::post('/test-email', [SystemSettingsController::class, 'testEmail'])->name('test-email');
    ...
    // Webhook Dashboard
    Route::prefix('webhooks')->name('webhook-dashboard.')->group(function () {
        Route::get('/', [App\Http\Controllers\Admin\WebhookDashboardController::class, 'index'])->name('index');
        Route::get('/stats', [App\Http\Controllers\Admin\WebhookDashboardController::class, 'stats'])->name('stats');
    });
    ...
    Route::prefix('notifications')->name('notifications.')->group(function () {
        ...
        Route::get('/history', [NotificationManagementController::class, 'history'])->name('history');        Route::get('/system-alerts', [NotificationManagementController::class, 'systemAlerts'])->name('system-alerts');
        Route::post('/system-alerts', [NotificationManagementController::class, 'storeSystemAlert'])->name('system-alerts.store');
