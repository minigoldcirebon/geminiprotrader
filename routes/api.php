...
// Webhook routes for external signals (with custom database-based rate limiting)
Route::prefix('webhook')->middleware(['webhook.rate'])->group(function () {
    // Receive new expert signals from external sources
    Route::post('/expert-signal', [WebhookController::class, 'expertSignal']);
    
    // Update signal results (close signals with profit/loss)
    Route::post('/signal-result', [WebhookController::class, 'updateSignalResult']);
});
